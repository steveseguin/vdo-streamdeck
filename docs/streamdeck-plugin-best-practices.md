...
- **Anti-alias generated raster art.** A hand-rolled rasteriser that writes whole pixels turns every circle, ring, and diagonal into visible stair-steps on the key LCD. Supersampling the edge pixels is enough and needs no image dependency — this repo renders each shape with 8×8 sub-samples inside its bounding box, which keeps the asset build offline- and CI-friendly.
- **Leave a title-safe zone on key state images.** Stream Deck draws the key title *over* the image, so a big centred glyph and a two-line title fight each other. Carry the state in a flat colour field, put the glyph in a corner badge, and leave the middle for the title. Check the contrast of white title text against every field colour.
- **Ship the key images larger than the documented pair when you can.** Stream Deck scales key images down to the target key resolution, so shipping 144/288 instead of 72/144 costs a few KB and avoids upscaling on higher-density hardware. `validate` accepts both.
- **Generate icons from one spec, then test the contract.** Hand-maintained SVG and PNG sets drift: this repo had list glyphs showing a microphone while the matching key tile showed a monitor, and eight generated tiles that no manifest field referenced. Deriving both formats from a single shape spec removes the drift, and a test that walks every manifest image path — asserting each file exists, is the expected size, and that nothing generated is unreferenced — keeps it that way.
...
- Keep glyphs simple: one concept per icon, 1.5–2px stroke weight at 20×20, consistent optical size across the set so the action list looks like one family. Check for duplicate metaphors — two actions that both render as faders are indistinguishable in the list.
